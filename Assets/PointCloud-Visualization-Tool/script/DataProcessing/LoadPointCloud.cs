...
public class LoadPointCloud
{
    ...
    static public int[] LoadInt(string filename)
    {
        try
        {
            using (FileStream fs = new FileStream(filename, FileMode.Open, FileAccess.Read))
...
public class csvController
{
    ...
    public Vector3[] StartLoad(string filename)
    {
        ...
        {
            vs[i - 1] = new Vector3(csvController.GetInstance().getFloat(i, 1),
                csvController.GetInstance().getFloat(i, 2), csvController.GetInstance().getFloat(i, 3));
        }
